...
`ShadowBacktest` 검증 직후와 `GNN Pruner` 추론 및 `Promotion Queue` 등록 단계 간의 결합을 분리하기 위해, 백테스트 성과(`backtest_result`), 피처(`features`), GNN 예측 점수(`gnn_score`)를 바인딩하는 중간 메모리 레코드 객체. feature drift 및 버전 미스매치 예방을 위해 `schema_version`, `feature_hash`, `regime_tag`, `generated_at`, `source_strategy_id` 스키마 필드를 의무적으로 가집니다.
...
물리적으로 저장된 Raw JSONL 파일로부터 부모 노드 정보(Parent State), N-hop 변이 체인 컨텍스트, 이전 ROI 트렌드(Previous ROI trend) 등의 파생 피처(Derived features)를 런타임에 동적으로 연산하여 가공하는 온디맨드 피처 파이프라인.
...
오프라인에서 전략 변이 DAG 정보 및 시장 국면을 그래프 어텐션 네트워크(GAT)를 통해 특징 압축한 뒤, 실시간 런타임에는 그래프 연산을 배제하고 오직 고정 차원의 요약 피처만을 입력받아 Single-head MLP 구조의 ONNX 모델로 10ms 이하 초고속 추론하는 리스크 평가 레이어. 오프라인 학습 시에는 2대 단순화 Loss를 활용하고, 모델 출력(`risk_score`, `uncertainty_score`)과 시스템 계산 유도(`confidence_score = 1 - uncertainty`, `stability_score = 0.30 * EMA(rank_change) + 0.25 * rolling_std(market_volatility) + 0.25 * feature_drift_score + 0.20 * system_latency_jitter`로 시스템의 지연/버퍼 지터를 20% 비중으로 격리 반영)를 분리 조율한 4중 지표 뷰를 가동하며, 런타임의 피처 정합성 오류 수준에 따라 가용성을 유지하기 위해 Staged Progressive Degradation 가드를 수행합니다.
...
**Promotion Event Log (승격 이벤트 로그)**:
Promotion Queue의 상태 변화와 승격 결정을 누적 기록하는 append-only 형태의 영속화 로그. 큐의 전체 랭킹 상태를 리플레이하여 복구할 수 있도록 `state = reduce(events)` 구조의 멱등성을 보장합니다.
...
실시간 랭킹 Fast Path의 지연 시간을 최소화하기 위해 동기 검증을 수행하지 않고, 백그라운드 데몬 루프에서 **주기적 배치 윈도우(e.g., 10s)** 단위로 `full_event_rebuild()`를 수행하며, 미세 랭킹 흔들림(Oscillation) 방지를 위해 이중 임계치 Hysteresis Rule ($T_{\text{low}}=0.1, T_{\text{high}}=0.3$), 상위권 가중 로그 가중치 $\text{weight\_i} = 1 / \log(\text{Rank}(i) + k)$ (k=2)와 후보 수 $N$에 따른 scale 변동을 예방하는 가중치합 정규화 기반 **Normalised Weighted Rank Drift** 오차, 그리고 **Single Decision Authority Rule** (Replay 부재 시 Sigmoid 스무딩 가중합 $\alpha = \text{sigmoid}(10 * (\text{stability\_score} - 0.5))$ 기반의 $\text{FINAL\_SCORE} = \alpha * girs\_promotion\_score + (1 - \alpha) * fallback\_promotion\_score$ 충돌 해소 적용, 모든 점수는 `1 - risk` 변환된 promotion_score로 통일)에 입각해 `Materialized View` 상태를 최종 보정하는 리플레이 안전 장치.
