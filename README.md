...
- **데몬 실시간 모니터링 및 제어**:
  - **수집기 데몬**: 수집 대기열(RCV/DB/Candle) 사용률 수준 진단, 메모리(RSS) 및 버전 감시, 비동기 시작/중지/재기동 제어가 지원되는 `🛰️ 수집기 데몬` 탭 제공.
  - **전략 데몬**: CPU/메모리 리소스, 실시간 판단 지연(Decision Latency), GIRS/Proposal 상태, 7대 가드레일 누적 차단 및 롤백 통계, 개별 전략 엔진들의 기동/Stale 여부 실시간 진단 및 비동기 재기동 제어가 완결된 `⚙️ 전략 데몬` 탭 제공.
...
| **시스템 아키텍처 및 클래스 명세** | [docs/architecture.md](file:///home/simon/ATS/docs/architecture.md) | 주요 파이썬 클래스 다이어그램, ZMQ IPC, 웹소켓 브로드캐스트 거시적/미시적 다이어그램 및 흐름 |
| **데이터베이스 및 ERD 명세** | [docs/database.md](file:///home/simon/ATS/docs/database.md) | SQLite 3 테이블 스키마, Mermaid ERD 관계도, 외래키 제약조건, 복합 인덱스 및 공간 최적화 Compact 정책 |
...
| **아키텍처 결정 기록 (ADR)** | [docs/adr/](file:///home/simon/ATS/docs/adr/) | 포트폴리오 매니저 분리 및 실시간 국소 복기·카운터팩츄얼 대조 기반 챔피언 전략 순환 매매 비전 선언(ADR 0008), 클린업 데몬 동시성 제어 및 ACK 프로토콜(ADR 0009), 자동 상장 및 상장폐지 감지/스케줄링 아키텍처(ADR 0010), 엄격한 가격 Hydration 및 튜플 키 조회 강제화(ADR 0011) 등 주요 결정 이력 |
