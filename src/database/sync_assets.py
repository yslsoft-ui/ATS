...
async def sync_exchange_assets(db_path: str) -> Dict[str, Any]:
    ...
    async with get_db_conn(db_path) as db:
        ...
        # 각 거래소별 동기화 대상 셋업
        targets = [
            ("upbit", upbit_symbols, "crypto", 1), # 업비트는 기본 활성
            ("bithumb", bithumb_symbols, "crypto", 1), # 빗썸은 기본 활성
            ("kis", kis_symbols, "stock", 0) # KIS 주식은 기본 비활성
        ]
        ...
        for exchange, api_list, asset_type, default_active in targets:
            ...
            # 3. 신규 종목 추가 프로세스
            for sym, name in api_list:
                api_symbols.add(sym)
                
                # 3.1 asset_master에 신규 종목 추가
