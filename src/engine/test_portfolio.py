...
async def test_portfolio_basic_operations():
    portfolio = Portfolio(portfolio_id="test_id", name="Test Portfolio")
    portfolio.exchange_cash = {"upbit": 10000000}
    portfolio.exchange_initial_cash = {"upbit": 10000000}
    
    # Buy operation (exchange_id 인자 주입)
    portfolio.update_position(exchange_id="upbit", symbol="KRW-BTC", side="BUY", price=50000000, quantity=0.01, fee=250)
    ...
    # Sell operation (exchange_id 인자 주입)
    portfolio.update_position(exchange_id="upbit", symbol="KRW-BTC", side="SELL", price=60000000, quantity=0.01, fee=300)
...
async def test_virtual_executor():
    portfolio = Portfolio(portfolio_id="sim_id", name="Simulation")
    portfolio.exchange_cash = {"upbit": 1000000}
    portfolio.exchange_initial_cash = {"upbit": 1000000}
    ...
    # Market BUY (exchange_id="upbit" 누락 매개변수 주입, portfolio 인자 제거)
...
async def test_portfolio_manager_handle_signal():
    ...
    portfolio = Portfolio(portfolio_id="default", name="Main Portfolio")
    portfolio.exchange_cash = {"upbit": 1000000}
    portfolio.exchange_initial_cash = {"upbit": 1000000}
    pm.add_portfolio(portfolio)
    await pm.save_to_db("default")
    
    # TradeSignal 생성자에 exchange_id="upbit" 누락 인자 주입
    signal = TradeSignal(exchange_id="upbit", symbol="KRW-BTC", action="BUY", price=50000000, reason="Test", interval=60)
    orderbook_data = {
        'asks': [[50000000, 10.0]],
        'bids': [[49000000, 10.0]]
    }
    ...
    # SELL signal handles all holdings (exchange_id="upbit" 누락 인자 주입)
    sell_signal = TradeSignal(exchange_id="upbit", symbol="KRW-BTC", action="SELL", price=50000000, reason="Test", interval=60)
...
async def test_portfolio_report_data_generation():
    ...
    pm = PortfolioManager(db_path=TEST_DB_PATH)
    portfolio = Portfolio(portfolio_id="test_report_id", name="Report Portfolio")
    portfolio.exchange_cash = {"upbit": 1000000}
    portfolio.exchange_initial_cash = {"upbit": 1000000}
    pm.add_portfolio(portfolio)
...
async def test_portfolio_manager_suspended_order_blocking():
    ...
    pm = PortfolioManager(db_path=TEST_DB_PATH)
    portfolio = Portfolio(portfolio_id="test_suspend_id", name="Suspend Portfolio")
    portfolio.exchange_cash = {"kis": 1000000}
    portfolio.exchange_initial_cash = {"kis": 1000000}
    pm.add_portfolio(portfolio)
    ...
    # 2. 주문 신호 생성 (kis)
    signal = TradeSignal(exchange_id="kis", symbol="005930", action="BUY", price=70000, reason="Test", interval=60)
