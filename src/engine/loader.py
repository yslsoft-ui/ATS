...
def load_dynamic_strategies(strategies_dir: str):
    ...
    loaded_count = 0
    logger.debug(f"Scanning directory: {strategies_dir}")
        ...
        if filename.endswith(".py") and not filename.startswith("__"):
            ...
            try:
                ...
                if spec is None or spec.loader is None:
                    logger.debug(f"Failed to create spec for {filename}")
                    continue
                ...
                loaded_count += 1
                logger.debug(f"Successfully loaded {filename}. Registered IDs: {registered_ids}")
