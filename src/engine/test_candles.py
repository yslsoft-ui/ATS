...
class TestCandleGenerator(unittest.TestCase):
    def test_candle_generation(self):
        ...
        # T=0ms: 첫 틱
        closed = generator.process_tick("upbit", symbol, 100, 1.0, "BID", 0)
        self.assertEqual(len(closed), 0)
        
        # T=500ms: 같은 1초 캔들 내 틱
        closed = generator.process_tick("upbit", symbol, 110, 1.0, "BID", 500)
        self.assertEqual(len(closed), 0)
        
        # T=1200ms: 1초 경과 -> 1초 캔들 완성
        closed = generator.process_tick("upbit", symbol, 105, 1.0, "BID", 1200)
        ...
        closed = generator.process_tick("upbit", symbol, 120, 1.0, "BID", 5500)
