...
### 2.7. KIS 백필 정합성 및 통합시세 가드
- **통합시세 조회 적용 (`UN` 마켓코드)**: 백필 캔들 요청 시 대체거래소(NXT) 야간시장 연장 거래(15:30 ~ 20:00)까지 데이터를 유실 없이 안전하게 포괄하기 위해 `"UN"`(통합시장) 조건 코드로 캔들 조회를 요청합니다.
- **미래 시간 조회 방지 가드 (20시 클리핑)**: KIS 분봉 조회 API(`FHKST03010200`)가 아직 도래하지 않은 미래 시간이나 거래가 없는 시간을 넘겨받을 시 에러 대신 최종 거래가와 거래량으로 가짜 캔들을 복사해서 돌려주는 오작동을 차단하기 위해, 조회 요청 기준 타임스탬프(`to_time`)를 최대 저녁 8시(20:00:00)로 강제 보정하여 호출합니다.
- **누적 거래대금 델타 필터**: API가 반환하는 캔들의 시간순(오름차순) 정렬 기준, 직전 캔들과 현재 캔들의 누적 거래대금(`acml_tr_pbmn`) 변화를 검사하여 거래대금 델타가 0원인 캔들(즉, 실제 체결 거래가 전혀 없었던 캔들)을 가짜 캔들로 간주하고 드롭(제외)하여 수집 데이터 무결성을 검증합니다.

### 2.8. 스케줄러 코어 및 설정식 기동 (ScheduleCalculator & SchedulerTrigger)

- **`ScheduleCalculator` (도메인 유틸리티)**:
  - 설정 파일(`settings.yaml`)에 지정된 스케줄 값들의 파싱 및 다음 실행 대기 시간 연산을 전담합니다.
  - 빗썸 공지사항 주기(`Hours` 단위) 및 업비트 마켓 리스트 감시 주기(`Hours` 단위)를 파싱하여 초(seconds)로 변환하는 `parse_hours_to_seconds` 및 `HH:MM` 포맷의 KIS 마스터 동기화 시각을 파싱하는 `parse_sync_time`을 제공합니다.
  - **Fail-Fast (Fail-Stop) 정책**: 설정값이 누락되었거나 유효하지 않은 포맷(예: 음수 시간, `HH:MM` 이외의 잘못된 포맷 문자열 등)일 경우 즉각 `ValueError`를 발생시켜 시스템이 잘못된 값으로 계속 동작하는 것을 원천 방지합니다.
- **`SchedulerTrigger` (실행기)**:
  - `CollectorService` 내부의 `while True` 무한 반복 대기 루프 및 예외 복구 로직을 캡슐화합니다.
  - 비동기 동작(`run_interval_loop`, `run_time_scheduled_loop`)을 수행하며, 루프 내부에서 `ValueError`가 발생하면 에러 로그를 적재하고 예외를 전파(Raise)시켜 데몬 프로세스를 안전하게 종료하도록 지원합니다.
  - 일반 실행 예외 발생 시에는 즉시 재시도하여 루프가 폭주하는 것을 막기 위해 60초간 대기 유예 후 재기동하는 자가 복구 정책을 제공합니다.

### 2.9. KIS 휴장일 조회 및 스케줄/수집 차단 필터

- **국내휴장일조회 API (`CTCA0903R`) 연동**:
  - 한국투자증권은 원장 서버 부하 방지를 위해 국내휴장일조회 API를 1일 1회만 호출하도록 강력히 권장합니다.
  - 이를 충족하기 위해 `CredentialProvider` 내부에 **오늘 날짜(YYYYMMDD) 기준 메모리 캐시(`kis_open_day_cache`)**를 설계하여, 하루 중 발생하는 복수 번의 조회 요청에 대해 실제 API 호출은 최초 1회만 수행되도록 통제합니다.
- **마스터 자산 동기화 스킵**:
  - 매일 07:50에 기동되는 KIS 마스터 동기화 루프(`_sync_kis_master_assets`) 초입에서 API를 찔러 오늘 날짜가 개장일(`opnd_yn == "Y"`)이 아닐 경우(주말 및 평일 공휴일 포함), 다운로드 및 DB 동기화 절차를 즉시 스킵합니다.
  - API 호출 예외 시 **Fail-Fast** 원칙에 따라 무작정 진행하지 않고 실행을 중단합니다.
- **수집기 (`KisCollector`) 연결 대기**:
  - 실시간 데이터 수집기 기동 시 `_pre_connect_check` 단계에서 오늘의 개장일 여부를 조회합니다.
  - 휴장일로 판명되면 익일 KST 00:00시까지 남은 대기 시간(초)을 계산하여 소켓 연결 루프를 장기 대기(sleep)시킴으로써 불필요한 소켓 재연결 시도와 트래픽 발생을 완전히 예방합니다.
  - 수집 루프의 영속성을 위해 API 호출 예외 발생 시에는 폭사하지 않고 60초 대기 후 안전하게 재시도하도록 설계되었습니다.
