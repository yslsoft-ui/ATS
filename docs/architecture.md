...
- **자동 상장 및 상장폐지 감지/선제 기동 스케줄러**:
  ...
  - **빗썸 (Bithumb)**: 6시간 주기로 공지사항 API를 폴링하며 신규상장/거래지원종료 공지 발견 시 본문 Next.js 상태값을 파싱하여 Ticker와 예정 시각을 정규식으로 추출해 `planned_asset_events` 테이블에 저장하고 실시간 ZMQ 토스트 알림을 발행합니다.
