...
## 9. Browser Testing Protocol (로컬 브라우저 테스트 규칙)
- 에이전트 기본 `browser_subagent` 도구는 가상 환경 샌드박스의 CDP 루프백(127.0.0.1) 연결 이슈로 로컬 웹 서버 검증 시 오작동할 수 있습니다.
- 따라서 웹 기능 및 프론트엔드 상태 검증이 필요한 경우, `browser_subagent` 도구 사용을 지양하고 가상환경(Venv)의 `playwright` 헤드리스 브라우저(Headless Chrome)를 활용한 검증 스크립트를 직접 작성·구동하여 테스트를 완료해야 합니다.
- 테스트 결과로 획득한 스크린샷 이미지 및 DOM 요소를 직접 확인하여 최종 렌더링 상태를 보증하십시오.
